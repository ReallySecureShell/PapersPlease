...
Additionally PapersPlease can **mass print** by passing the `--papyrus` flag, which can be used with either `--network` or `--target`. With this comes two other flags that are only valid when using mass print. The first being the `--message`, which can print a **custom message** to the print document. And the second is the `--ink` flag, which specifies how many pound symbols (#) are to be printed to the document. Finally, to **disable all address restrictions**, pass the `--allow-all` flag. By specifying said flag all public and private IP addresses will be allowed including all CIDR prefixes less-than or equal to 30.
